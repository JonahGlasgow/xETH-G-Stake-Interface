...
export const STAKING_GENESIS = 1607935622
  ...
  return useMemo(() => {
    ...
    return rewardsAddresses.reduce<StakingInfo2[]>((memo, rewardsAddress, index) => {
      ...
      if (
        ...
        //const dummyPair = new TokenAmount(xethg, '0')
        

        // check for account, if no account set to 0

        const stakedAmount = new TokenAmount(xethg, JSBI.BigInt(balanceState?.result?.[0] ?? 0))
        const totalStakedAmount = new TokenAmount(xethg, JSBI.BigInt(totalSupplyState.result?.[0]))
